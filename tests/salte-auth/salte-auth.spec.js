...
describe('salte-auth', () => {
  ...
  describe('function: getCachedUser', () => {
    it('should return the cached user', () => {
      ...
      expect(auth.getCachedUser()).toEqual({
  ...
  describe('function: login', () => {
    ...
    it('should navigate the user to login by default', () => {
      ...
      expect(auth.promptUser.calledWith('https://login.microsoftonline.com/tenant/oauth2/authorize' +
        ...
        '&nonce=33333333-3333-4333-b333-333333333333')).toEqual(true);
    ...
    it('tests if callback is called after login, if popup window is null', () => {
      ...
      expect(callback.calledWith('Popup Window is null. This can happen if you are using IE', null)).toEqual(true);
  ...
  describe('function: acquireToken', () => {
    afterEach(() => {
    ...
    it('returns from cache for auto renewable if not expired', () => {
      ...
      expect(callback.callCount).toEqual(1);
      expect(callback.calledWith(null, 'access_token_in_cache' + RESOURCE1)).toEqual(true);
    });
    ...
    it('returns error for acquireToken without resource', () => {
      ...
      expect(callback.callCount).toEqual(1);
      expect(callback.calledWith('resource is required', null)).toEqual(true);
    });
    ...
    it('attempts to renew if token expired and renew is allowed', (done) => {
      ...
      expect(sessionStorage.getItem(auth.CONSTANTS.STORAGE.LOGIN_REQUEST)).toBe('');
      ...
      setTimeout(() => {
        expect(document.getElementById('authRenewFrame' + RESOURCE1).src).toEqual('https://login.microsoftonline.com/tenant/oauth2/authorize' +
          '?response_type=token' +
          '&client_id=client' +
          '&resource=' + RESOURCE1 +
          '&redirect_uri=contoso_site' +
    ...
    it('allows multiple callers to be notified when the token is renewed. Also checks if all registered acquireToken callbacks are called in the case when one of the callbacks throws an error', (done) => {
      ...
      setTimeout(() => {
        expect(callback.callCount).toEqual(1);
        expect(callback.calledWith(null, '33333333-3333-4333-b333-333333333333')).toEqual(true);

        expect(callback2.callCount).toEqual(1);
        expect(callback2.calledWith(null, '33333333-3333-4333-b333-333333333333')).toEqual(true);
        expect(document.getElementById('authRenewFrame' + RESOURCE1).src).toEqual('https://login.microsoftonline.com/tenant/oauth2/authorize' +
          '?response_type=token' +
          '&client_id=client' +
          '&resource=' + RESOURCE1 +
    ...
    it('tests that callbacks are called when renewal token request was canceled', (done) => {
      ...
      const callback = (error, token) => {
        expect(sessionStorage.getItem(auth.CONSTANTS.STORAGE.RENEW_STATUS + RESOURCE1)).toBe(auth.CONSTANTS.TOKEN_RENEW_STATUS_CANCELED);
        expect(error).toBe('Token renewal operation failed due to timeout');
    ...
    it('attempts to renewidToken if token expired and renew is allowed', (done) => {
      ...
      expect(sessionStorage.getItem(auth.CONSTANTS.STORAGE.NONCE_IDTOKEN)).toBe('33333333-3333-4333-b333-333333333333');
      expect(auth.config.state).toBe('33333333-3333-4333-b333-333333333333|client');
      expect(auth._renewStates.length).toBe(1);
      expect(sessionStorage.getItem(auth.CONSTANTS.STORAGE.LOGIN_REQUEST)).toBe('');

      setTimeout(() => {
        expect(document.querySelector('#authIdTokenFrame').src).toEqual('https://login.microsoftonline.com/tenant/oauth2/authorize?response_type=id_token&client_id=' + auth.config.clientId + '&redirect_uri=contoso_site&state=33333333-3333-4333-b333-333333333333%7Cclient' +
  ...
  describe('function: promptUser', () => {
    it('prompts user if url is given', () => {
      sessionStorage.setItem(auth.CONSTANTS.STORAGE.USERNAME, 'test user');
      auth.promptUser();
      expect(auth.navigate.callCount).toEqual(0);
      auth.promptUser('test');
      expect(auth.navigate.callCount).toEqual(1);
    });
    ...
    it('checks if Logging is defined on window', () => {
      ...
      expect(window.Logging.log.callCount).toEqual(1);
      expect(window.Logging.log.getCall(0).args[0]).toContain('Navigate url is empty');
    });
  });
  ...
  describe('function: clearCache', () => {
    it('clears cache', () => {
      ...
      for (let i = 0; i < sessionStorage.length; i++) {
        expect(Boolean(sessionStorage[i])).toEqual(false);
      }
    });
    ...
    it('clears cache for a resource', () => {
      ...
      expect(sessionStorage[auth.CONSTANTS.STORAGE.ACCESS_TOKEN_KEY + RESOURCE1]).toEqual('');
      expect(sessionStorage[auth.CONSTANTS.STORAGE.EXPIRATION_KEY + RESOURCE1]).toEqual('0');
      expect(sessionStorage[auth.CONSTANTS.STORAGE.ACCESS_TOKEN_KEY + 'key1']).toEqual('value1');
    });
  });
  ...
  describe('function: logOut', () => {
    ...
    it('clears cache before logout', () => {
      auth.config.redirectUri = 'contoso_site';
      auth.logOut();
      expect(auth.clearCache.callCount).toEqual(1);
      expect(auth.promptUser.callCount).toEqual(1);
    });
    ...
    it('should redirect after logout if provided', () => {
      ...
      auth.logOut();
      expect(auth.promptUser.callCount).toEqual(1);
      expect(auth.promptUser.calledWith('https://login.microsoftonline.com/tenant/oauth2/logout?post_logout_redirect_uri=https%3A%2F%2Fcontoso.com%2Flogout')).toEqual(true);
    });
  });
  ...
  describe('function: getUser', () => {
    ...
    it('ensures that auth.callback is not overridden in calls to getUser', () => {
      ...
      expect(callback.callCount).toEqual(1);
      expect(callback.calledWith(null, auth._user)).toEqual(true);
  ...
  describe('function: getRequestInfo', () => {
    it('gets request info from hash', () => {
      ...
      expect(requestInfo.valid).toBe(true);
      expect(requestInfo.stateResponse).toBe('1234');
      expect(requestInfo.stateMatch).toBe(true);
      expect(requestInfo.requestType).toBe(auth.REQUEST_TYPE.LOGIN);
  ...
  describe('function: saveTokenFromHash', () => {
    it('saves errors token from callback', () => {
      ...
      expect(sessionStorage.getItem(auth.CONSTANTS.STORAGE.ERROR)).toBe('invalid');
      expect(sessionStorage.getItem(auth.CONSTANTS.STORAGE.ERROR_DESCRIPTION)).toBe('error description');
    });
    ...
    it('saves token if state matches', () => {
      ...
      expect(sessionStorage.getItem(auth.CONSTANTS.STORAGE.ACCESS_TOKEN_KEY + 'loginResource1')).toBe('token123');
    });
    ...
    it('saves expiry if state matches', () => {
      ...
      auth.saveTokenFromHash(requestInfo);
      expect(sessionStorage.getItem(auth.CONSTANTS.STORAGE.EXPIRATION_KEY + 'loginResource1')).toBe('7178');
    });
    ...
    it('saves null for username if idtoken is invalid', () => {
      ...
      expect(sessionStorage.getItem(auth.CONSTANTS.STORAGE.USERNAME)).toEqual(null);
    });
    ...
    it('saves error if state mismatch', () => {
      ...
      expect(sessionStorage.getItem(auth.CONSTANTS.STORAGE.ERROR_DESCRIPTION)).toBe('Invalid_state. state: ' + requestInfo.stateResponse);
    });
  });
  ...
  describe('function: _loadFrameTimeout', () => {
    let originalTimeout;
    beforeEach(() => {
    ...
    it('tests the load frame timeout method', (done) => {
      window.callBackMappedToRenewStates['33333333-3333-4333-b333-333333333333|token.resource1'] = function() {
        ...
        expect(sessionStorage.getItem(auth.CONSTANTS.STORAGE.RENEW_STATUS + RESOURCE1)).toBe(auth.CONSTANTS.TOKEN_RENEW_STATUS_IN_PROGRESS);
        const requestInfo = {
          valid: true,
        ...
        auth.saveTokenFromHash(requestInfo);
        expect(sessionStorage.getItem(auth.CONSTANTS.STORAGE.RENEW_STATUS + RESOURCE1)).toBe(auth.CONSTANTS.TOKEN_RENEW_STATUS_COMPLETED);
        done();
      };
  ...
  describe('function: handleWindowCallback', () => {
    it('tests handleWindowCallback function for RENEW_TOKEN', (done) => {
      function callback(error, token) {
        expect(error).toEqual('error description');
        expect(token).toEqual(IDTOKEN_MOCK);
        done();
      }
    ...
    it('tests handleWindowCallback function for LOGIN_REQUEST', () => {
      ...
      auth.handleWindowCallback();
      expect(auth.navigate.calledWith('www.test.com')).toEqual(true);
    });
  });
  ...
  describe('function: _initResponseType', () => {
    it('verifies _getNavigateUrl() returns the correct value when tenant and rootContext are undefined, and scope and responseType are not truthy', () => {
      ...
      expect(auth._getNavigateUrl(auth.config.responseType, '')).toMatch(/https:\/\/login\.microsoftonline\.com\/tenant\/oauth2\/authorize\?response_type=id_token&client_id=the_client_id&redirect_uri=the_redirect_uri&state=the_state&client-request-id=the_correlation_id.*/);
    });
