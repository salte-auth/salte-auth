...
global.window = {};
var AdalModule = require('../../src/salte-auth.js');
...
describe('Adal', () => {
  ...
  it('navigates user to login by default', () => {
    ...
    sinon.spy(adal, 'promptUser');
    adal.login();
    expect(adal.promptUser.calledWith(DEFAULT_INSTANCE + conf.tenant + '/oauth2/authorize?response_type=id_token&client_id=client&redirect_uri=contoso_site&state=33333333-3333-4333-b333-333333333333' +
      '&client-request-id=33333333-3333-4333-b333-333333333333' + adal._addLibMetadata() + '&nonce=33333333-3333-4333-b333-333333333333')).toEqual(true);
  ...
  it('calls displaycall if given for login', () => {
    ...
    adal.config.clientId = 'client';
    adal.config.redirectUri = 'contoso_site';
    adal.config.displayCall = sinon.spy();
  ...
  it('attempts to renew if token expired and renew is allowed', (done) => {
    ...
    adal.acquireToken(RESOURCE1, sinon.spy());
    ...
    setTimeout(() => {
      expect(mockFrames['adalRenewFrame' + RESOURCE1].src).toBe(DEFAULT_INSTANCE + conf.tenant + '/oauth2/authorize?response_type=token&client_id=client&resource=' + RESOURCE1 +
  ...
  it('gets user from cache', function() {
    ...
    sinon.spy(adal, 'getCachedToken');
    adal.getUser(callback);
    expect(adal.getCachedToken.calledWith(RESOURCE1)).toEqual(false);
  ...
  it('attempts to renewidToken if token expired and renew is allowed', function(done) {
    ...
    adal._renewStates = [];
    adal._user = { profile: { upn: 'test@testuser.com' }, userName: 'test@domain.com' };
    adal.acquireToken(adal.config.clientId, sinon.spy());
